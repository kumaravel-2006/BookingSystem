...
const EventDetails = ({ navigateTo, selectedEvent, onSelectShowtime }) => {
  // If no event is selected (e.g. direct url refresh), fallback to home
  if (!selectedEvent) {
    return (
  ...
  const handleBookingStart = () => {
    ...
    // Determine where to route based on demand
    if (selectedEvent.isHighDemand) {
      navigateTo('waiting-queue');
    } else {
      ...
      <div className="glass-panel" style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '3rem', padding: '2.5rem' }}>
        ...
        <div style={{ flex: '1 1 300px', maxWidth: '360px' }}>
          <div className="poster-wrapper" style={{ borderRadius: '12px', boxShadow: 'var(--shadow-md)' }}>
            <img src={selectedEvent.poster} alt={selectedEvent.title} className="movie-poster" />
        ...
        <div style={{ flex: '2 2 400px', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div>
            ...
            <h1 style={{ fontSize: '2.5rem', marginBottom: '0.5rem', lineHeight: '1.1' }}>{selectedEvent.title}</h1>
            <p style={{ fontSize: '1.05rem', color: 'var(--primary-hover)', fontWeight: '600' }}>
              {selectedEvent.genre} &bull; {selectedEvent.duration} &bull; {selectedEvent.format}
            </p>
          </div>
          ...
          <div style={{ borderTop: '1px solid var(--border-color)', borderBottom: '1px solid var(--border-color)', padding: '1.25rem 0' }}>
            ...
            <p style={{ margin: 0, lineHeight: '1.6', fontSize: '0.95rem' }}>{selectedEvent.description}</p>
          </div>
          ...
          {selectedEvent.cast && (
            <div>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                {selectedEvent.cast.map((member, index) => (
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem', borderTop: '1px solid var(--border-color)', paddingTop: '1.5rem' }}>
              <div>
                ...
                <div style={{ fontSize: '1.5rem', fontWeight: '800', color: 'var(--text-bright)' }}>
                  ${selectedEvent.ticketPrice.toFixed(2)} <span style={{ fontSize: '0.85rem', fontWeight: '400', color: 'var(--text-muted)' }}>/ seat</span>
                </div>
              </div>
